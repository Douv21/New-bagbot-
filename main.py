...
SECRET_KEY = os.getenv("SECRET_KEY", "default_secret_if_none") # Clé pour les sessions

app = Flask(__name__, static_folder='public', static_url_path='')
CORS(app)

# Configuration de la session (mémoire côté serveur)
...
# --- ROUTES AUTHENTIFICATION (OAUTH2) ---

@app.route('/login')
def login():
    # Lien de connexion vers Discord
    discord_login_url = f"https://discord.com/api/oauth2/authorize?client_id={CLIENT_ID}&redirect_uri={REDIRECT_URI}&response_type=code&scope=identify+guilds.members.read"
...
def callback():
    ...
    if not access_token:
        return "Erreur d'authentification (Pas de token reçu).", 400

    # Vérification si l'utilisateur est bien sur ton serveur
    ...
    return "Accès refusé : Vous ne possédez pas les rôles requis sur le serveur.", 403
